...
(() => {
  ...
  async function render() {
    ...
    root.querySelectorAll('[data-edit-file]').forEach(button => button.onclick = () => editFile(files.find(file => file.id === button.dataset.editFile)));
  ...
  async function upload(event, form, dialog) {
    ...
    const fileUrl = client.storage.from('course-files').getPublicUrl(path).data.publicUrl; const { data: { user } } = await client.auth.getUser(); const { error: saveError } = await client.from('course_files').insert({ course_id: courseId, category: form.elements.category.value, name: form.elements.name.value.trim(), file_name: file.name, file_url: fileUrl, created_by: user.id }); if (saveError) { msg.className = 'form-message form-message-error'; msg.textContent = saveError.message; return; }
    dialog.close(); render();
  }
  ...
  async function editFile(file) {
    if (!file) return;
    const name = window.prompt('Nama file:', file.name);
